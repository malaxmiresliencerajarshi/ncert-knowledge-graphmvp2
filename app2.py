...
for item in data:
    ...
    grade = str(item.get("grade", "7"))
    
    if source and source not in nodes_added:
        nodes.append(Node(id=source, label=source, size=12, color=grade_colors.get(grade, "#6495ED")))
        nodes_added.add(source)
    
    if target and target not in nodes_added:
        nodes.append(Node(id=target, label=target, size=25, color=hub_color, shape="diamond"))
        nodes_added.add(target)
        
    if source and target:
        edges.append(Edge(source=source, target=target, label=item.get("relation", "")))

# 3. THE UI
st.title("🌌 The NCERT Knowledge Graph")
st.subheader("Galaxy Model: Conceptual Hubs & Links")

# DEBUGGER: If you see this text, the code is working!
